...
void application::create_button_outer_grid(const char *button_text,
    int left,
    int top,
    int width,
    int height,
  ...
  GtkGrid *out_grid = (GtkGrid *)get_object("OuterGrid");
  ...
  // add the new button
  gtk_grid_attach(out_grid, new_button, left, top, width, height);
...
bool application::destroy_button_outer_grid(const char *button_text_to_destroy)
{
  // get the inner grid
  GtkGrid *out_grid = (GtkGrid *)get_object("OuterGrid");
  ...
  // iterate over all of the children in the grid and find the button by it's text
  GList *children, *iter;
  children = gtk_container_get_children(GTK_CONTAINER(out_grid));
  for(iter = children; iter != NULL; iter = g_list_next(iter)) {
    // iterator to widget
    GtkWidget *widget = GTK_WIDGET(iter->data);
